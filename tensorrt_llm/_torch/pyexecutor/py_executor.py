...
class PyExecutor:
    ...
    def _append_iter_stats(self,
        ...
        scheduler/KV-cache state. This method collectively gathers every
        rank's :class:`IterationStats` (+ optional per-request stats and KV
        iteration stats) via :meth:`tp_allgather` and rank 0 stores one
        pre-serialized JSON dict per rank (tagged with ``"rank"``) in
        ``self.stats`` so ``/metrics`` can export stats from every rank.
        Non-leader ranks drop the gathered result.

        Under pure TP (no attention-DP) every rank runs the same requests
        on the same iteration, so the gather would be redundant and only
        adds a CPU-GPU sync on the hot path — we fall through to the
        legacy rank-0-only append.
        ...
        # Per-rank stats gathering: when attention-DP is enabled (ranks have
        # diverging scheduler/KV-cache state), serialize each rank's stats
        # to a JSON dict and allgather so rank 0 can export all ranks' stats
        # via /metrics. Under pure TP (no attn_dp), every rank processes the
        # same requests on the same iteration, so per-rank stats would be
        # redundant and the allgather just adds a CPU-GPU sync on the hot
        # path — skip it.

...
class OpenAIDisaggregatedService(OpenAIService):
    ...
    def _patch_gen_cached_tokens(self, gen_result, ctx_response):
        """Replace GEN worker's cached_tokens with CTX worker's real prefix-cache reuse."""
    ...
    async def _send_disagg_request_ctx_first(
        ...
        if ctx_response is None or self._need_gen(ctx_response):
            ...
            return self._patch_gen_cached_tokens(gen_result, ctx_response)
        else:
            if request.stream:
    ...
    async def _send_disagg_request_gen_first(
        ...
        if need_ctx and len(responses) > 1:
            gen_result = self._patch_gen_cached_tokens(gen_result, responses[0])
        return gen_result
